...
NS_LOG_COMPONENT_DEFINE ("TCP-Evalvid-Multiflows1");
...
main (int argc, char *argv[])
{
    ...
    uint64_t simulationTime = 50; //seconds
     ...
     mac.SetType ("ns3::ApWifiMac",
      ...
      /* Setting mobility model */
       ...
       mobility.SetPositionAllocator ("ns3::GridPositionAllocator",
        ...
        string traceFile[3] = {"st_highway_cif.st", "st_waterfall_cif.st", "st_flower_cif.st"}; 
        for (int i = 1; i <= 3 ;i++)
        {
                // Create a EvalvidServer and install it on node csmaNodes(nCsma-(i-1))
                //EvalvidServer Send video 
                uint16_t port = 400;  
                port += i;

                EvalvidServerHelper source ("ns3::TcpSocketFactory",
                                         InetSocketAddress (StaInterface.GetAddress (nWifi-i), port));
                // Set the amount of data to send in bytes.  Zero is unlimited.
                source.SetAttribute ("MaxBytes", UintegerValue (maxBytes));
                source.SetAttribute ("SendSize", UintegerValue (tcp_adu_size));
                source.SetAttribute ("SenderTraceFilename", StringValue(traceFile[i-1]));
                string SenderTraceFilename = "sender-output";
                string ch = itos(i);
                SenderTraceFilename += ch;
                source.SetAttribute ("SenderDumpFilename", StringValue(SenderTraceFilename));
                string SendSizeFileName = "send_size";
                string ch1 = itos(i);
                SendSizeFileName += ch1;
                source.SetAttribute ("SendSizeFileName", StringValue(SendSizeFileName));
                ApplicationContainer sourceApps = source.Install (csmaNodes.Get (nCsma-(i-1)));

                sourceApps.Start (Seconds (0.0));
                sourceApps.Stop (Seconds (simulationTime));


                // Create a EvalvidClient and install it on node wifiStaNodes(nWifi-i)
                EvalvidClientHelper sink ("ns3::TcpSocketFactory",
                                         InetSocketAddress (Ipv4Address::GetAny (), port));
                string ReceiverDumpFilename = "receiver-output";
                string ch2 = itos(i);
                ReceiverDumpFilename += ch2;
                sink.SetAttribute ("ReceiverDumpFilename", StringValue(ReceiverDumpFilename));
                sink.SetAttribute ("SendSizeFileName", StringValue(SendSizeFileName));
                  
                ApplicationContainer sinkApps = sink.Install (wifiStaNodes.Get (nWifi-i));
