...
int main (int argc, char *argv[])
{
  ...
  /*by fakhrali fo extracting bitrate probabilities*/
/*  std::fstream fs("prRates.txt", std::fstream::out);
  NS_ASSERT(fs.is_open());
  for (uint8_t i = 0; i< nSta; i++)
    {
      std::map<uint32_t,double> McsProbability;
      Mac48Address  MacAddressOfRemoteStation = wifiStaNode.Get(i)->GetDevice(0)->GetObject<WifiNetDevice>()->GetMac()->GetAddress();
      uint8_t tid=0; //tid=0 means AC_BE
      //Get remote station for MacAddressOfRemoteStation
      WifiRemoteStation *station = wifiApNode.Get (0)->GetDevice(0)->GetObject<WifiNetDevice>()->GetRemoteStationManager()->
          Lookup(MacAddressOfRemoteStation,tid);
      McsProbability=wifiApNode.Get (0)->GetDevice(0)->GetObject<WifiNetDevice>()->GetRemoteStationManager()->
          GetObject<IdealWifiManagerForMarkovChannelModel11n>()->CalculateTheProbabilityOfEachMCS (station);
      std::map<uint32_t,double>::iterator it;
      for (it = McsProbability.begin(); it != McsProbability.end(); ++it)
        {
          fs << " " << it->second << " ";
        }
      fs << "\n";
    }
  uint32_t bitrates[32] = { 6500000, 13000000, 19500000, 26000000, 39000000, 52000000, 58500000, 65000000, 13000000, 26000000, 39000000,
  52000000, 78000000, 104000000, 117000000, 130000000, 19500000, 39000000, 58500000, 78000000, 117000000, 156000000, 175500000, 195000000, 26000000,
  52000000, 78000000, 104000000, 156000000, 208000000, 234000000, 260000000};
  for (int i = 0; i < 24; ++i)
    {
      fs << " " << bitrates[i] << " ";
    }
