...
EvalvidServer::Setup()
{
    ...
    while (m_videoInfoMapIt != m_videoInfoMap.end())
    {
         ...
         if (s > 0)
             m_sendsizeFile << s+12 <<"\t"<<m_videoInfoMapIt->second->packetInterval.GetSeconds() <<std::endl;
         m_videoInfoMapIt++;
     }
    ...
    output.open("sender-output2.txt", ios::out);
          ...
          if (InetSocketAddress::IsMatchingType (m_peer))
            {
              NS_LOG_DEBUG(">> EvalvidServer: Send packet at " << Simulator::Now().GetSeconds() << "s\tid: " << m_packetId
                            << "\tudp\t" << p->GetSize() << " to " << InetSocketAddress::ConvertFrom (m_peer).GetIpv4 ()
                            << std::endl);
            }
          else if (Inet6SocketAddress::IsMatchingType (m_peer))
            {
              NS_LOG_DEBUG(">> EvalvidServer: Send packet at " << Simulator::Now().GetSeconds() << "s\tid: " << m_packetId
                            << "\tudp\t" << p->GetSize() << " to " << Inet6SocketAddress::ConvertFrom (m_peer).GetIpv6 ()
                            << std::endl);
            }
      ...
      if (m_videoInfoMapIt == m_videoInfoMap.end())
      ...
      else
        {
          if (m_videoInfoMapIt->second->packetInterval.GetSeconds() == 0)
            ...
            {
              m_sendEvent = Simulator::Schedule (Seconds(0.01),
                                                 &EvalvidServer::Send, this);//m_videoInfoMapIt->second->packetInterval
            }
        }
    }
  else
...
void EvalvidServer::SendData (void)
{
    ...
    if (m_tid == TcpSocketFactory::GetTypeId ())
    {
     ...
     else if (m_tid == UdpSocketFactory::GetTypeId ())
     {
          ...
          if (InetSocketAddress::IsMatchingType (m_peer))
          {
               std::cout<< Simulator::Now().GetSeconds() <<" s "<< m_packetId <<" "<< "Data "<< p->GetSize()+12 <<" " << InetSocketAddress::ConvertFrom(m_peer).GetIpv4 ()<< " udp "<<p_Interval.GetSeconds()<<"\n";

               output<< Simulator::Now().GetSeconds() <<" s "<< m_packetId <<" "<< "Data "<< p->GetSize()+12 <<" " << InetSocketAddress::ConvertFrom(m_peer).GetIpv4 ()<< " udp "<<p_Interval.GetSeconds()<<"\n";
